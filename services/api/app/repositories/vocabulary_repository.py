...
class VocabularyRepository:
    ...
    def _paraphrase_from_row(
        ...
        *,
        ...
        export_row = connection.execute(
            "SELECT exported_at FROM paraphrase_exports WHERE user_id = ? AND paraphrase_id = ?",
            (row["user_id"], row["id"]),
        ).fetchone()
        return {
            "id": str(row["id"]),
        ...
        with self._connect() as connection:
            ...
            if existing:
                ...
                connection.execute(
                    ...
                    SET note = ?, confidence = ?, updated_at = ?
                    WHERE id = ?
                    """,
                    (next_note, next_confidence, now, item_id),
                )
            else:
                item_id = uuid.uuid4().hex
                connection.execute(
                    ...
                    INSERT INTO paraphrase_items (
                        ...
                        confidence, occurrence_count, created_at, updated_at
                    ) VALUES (?, ?, ?, ?, ?, ?, ?, 'learning', ?, 0, ?, ?)
                    """,
                    (
                        item_id,
    ...
    def mark_paraphrases_exported(self, *, user_id: str, item_ids: list[str]) -> int:
        ...
        with self._connect() as connection:
            for item_id in ids:
                connection.execute(
                    """
                    INSERT INTO paraphrase_exports (user_id, paraphrase_id, exported_at)
                    VALUES (?, ?, ?)
